...
// ── GET /api/jobs/all/logs — fetch all job logs ─────────────────────
exports.getAllJobs = async (req, res) => {
  try {
    const jobs = await JobService.getAllJobLogs();
    return res.json({ count: jobs.length, jobs });
  } catch (err) {
    console.error("[API] Job logs fetch error:", err.message);
...
exports.deadLetterQueueList = async (req, res) => {
  try {
